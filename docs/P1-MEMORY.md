...
| 2026-08-05 | **`n_prompts` = 256 total, 128 discovery**, selected by the committed calibration rule, not chosen | `J_seed` = 0.5678 / 0.6386 / 0.7428 / 0.8089 at n = 16/32/64/128; no delta below the pre-committed 0.05, so the curve had not flattened and the rule fell back to the largest size tested, which is also auto-circuit's default. Two runs of the identical specification agree on only 81% of a 500-edge circuit, so seed is a comparison arm, not a nuisance term. State the non-flattening in the abstract. |
| 2026-08-05 | **GPU factor measured on Tesla T4: ~7x, not D17's guessed 20x** | EAP 6.95x, IEG-50 8.37x, evaluation 6.97x. IEG gains more because 51 passes keep the device busier than 1. Instrument versions identical on both sides; only torch differs, 2.13.0 against 2.11.0+cu128. |
| 2026-08-05 | **D5 reversed: sweep runs on Colab Pro, no rented box** | Peak VRAM 3,651 MB of 15,360 at the selected dataset size, 24% of the card. Budget 25.7 h on measured numbers. The original objection, that ephemeral filesystems make an environment hash meaningless, is answered by recording the device in every cell manifest and enforcing homogeneity, which is more evidence than a single asserted hash, not less. |
| 2026-08-05 | **`tau` search fixed: smallest rung of `[10,20,50,100,200,500,1000,2000,5000,10000]`** | The definition named a criterion but never a search, so `C(s)` was underdetermined and a different search meant a different claim. Bisection rejected on soundness: metric recovery is not guaranteed monotone in edge count. Top rung deliberately below the 32,491-edge full model, so a specification needing more than 10,000 edges is discarded rather than handed a degenerate whole-model circuit. |
| 2026-08-05 | **Resume: per-cell manifest, skip if `status: ok`** | 25.7 h exceeds a Colab session, so resume is mandatory. The results directory is the checkpoint; no separate state file to corrupt mid-write. Device recorded per cell, non-modal cells discarded and rerun, distribution reported. |
