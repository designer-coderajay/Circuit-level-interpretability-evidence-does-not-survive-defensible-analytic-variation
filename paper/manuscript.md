...
across **73.2%** of specification pairs (95% CI 0.725 to 0.738) and the modal
claim commands **41.1%** of the space. The evidence fails a filability criterion
...
the flip rate at **59.4%**. Removing circuit size from the claim entirely and
holding it fixed leaves **27.1%** (95% CI 0.255 to 0.286), still above the
pre-registered threshold. The underlying circuits are structurally near-disjoint,
median pairwise Jaccard overlap **4%**, and functionally uncorrelated, Cohen's
kappa **0.015**, so the instability is not one mechanism described in different
words.
...
library's own canonical task. **The study covers one model and one task; whether
...
Nobody has connected these to the filing. That is the gap this paper closes.

**Contribution.** We measure how far a regulatory claim moves across the space of
defensible analytic specifications, and we give the measurement a decision rule
that a standards body can apply. We do not propose a better circuit discovery
...
**Not that discovered circuits are no better than random.** At fixed circuit size
they are more stable than a size-matched random null, 0.275 against 0.423. They
carry information.

**Not that filings differ where mechanisms do not.** Mechanisms differ
substantially: functional instability is 0.397 pooled and 0.478 at the smallest
circuits.

**Not that circuit discovery samples one mechanism and dresses it differently.**
Cohen's kappa of 0.015 says the circuits are functionally uncorrelated.

**Not that random seeds are the problem.** In this design the seed selects which
prompts are sampled, so it is evaluation-set variability, not nondeterminism.

**Nothing about scale.** One model, one task.
...
`phi_overseer` serves Annex IV 2(e) and 3 with Article 14(4)(c), and is keyed on
the dominant layer band, then the size class, then the ranked input segment.
`phi_affected` serves Article 86(1) and leads with the input segment, because the
...
**The tuning objection, answered by measurement.** A reviewer will say the size
...
The distribution says more than the mean. **The median pair of circuits shares 4%
of its edges.** Only 1% of pairs exceed a Jaccard of 0.8. At the median circuit
...
**We do not report that as a finding, because it is an artefact of the size
distribution.** The null is degenerate above 1,000 edges, where a uniform draw
...
**Not verified. Must not survive to submission.**
