...
def main() -> int:
    ...
    sizes = cached["sizes"]
    discovered = cached["labels"][GRANULARITY]

    index = EdgeComponentIndex()
    gran_pos = [g.name for g in NULL_GRANULARITIES].index(GRANULARITY)

    null_labels = np.empty((R_DIAG, len(sizes)), dtype=object)
    for r in range(R_DIAG):
        rng = _replicate_rng(r)
        for i, k in enumerate(sizes):
            null_labels[r, i] = index.claims(index.sample(k, rng), NULL_GRANULARITIES)[
                gran_pos
            ]
    ...
    for k in ladder:
        ...
        n_vals = [flip_rate([null_labels[r, i] for i in idx]) for r in range(R_DIAG)]
        rows.append(
            {
                "size": k,
                ...
                "null_classes_median": float(
                    np.median([len({null_labels[r, i] for i in idx}) for r in range(R_DIAG)])
                ),
            }
        )

    # Same quantity pooled, with size held fixed, so it is comparable to stage 3.
    pooled = {
        "discovered_F_within_size": within_flip_rate(discovered, sizes),
        "null_F_within_size_median": float(
            np.median(
                [within_flip_rate(list(null_labels[r]), sizes) for r in range(R_DIAG)]
            )
        ),
    }
    ...
    report = {
        ...
        "purpose": "locate the source of the null's stability across circuit size",
        "granularity": GRANULARITY,
        "R": R_DIAG,
        "by_size": rows,
        "with_size_held_fixed": pooled,
    }
    ...
    print(
        f"\nsize held fixed:  discovered {pooled['discovered_F_within_size']:.4f}"
        f"   null {pooled['null_F_within_size_median']:.4f}"
    )
